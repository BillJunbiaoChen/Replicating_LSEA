   ...
   for (j_id,J) in enumerate(J_to_consider)
      for (pop_id,Pop) in enumerate(Pops_to_consider)
         ...
         Threads.@threads for k in 1:n_iter
            ...
            println("Data/simulated_choices_true_prob_mat_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)/prob_mat_J$(J)_Pop$(Pop)_iter$(k).csv")
            println(pwd())
