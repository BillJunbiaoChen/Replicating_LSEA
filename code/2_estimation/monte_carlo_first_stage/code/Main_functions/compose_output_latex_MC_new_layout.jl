...
for include_tau_dummy in tau_to_consider
   for FEs in FEs_to_consider
      for divide_by_params in divide_by_params_options
         ...
         df_output_zero = DataFrame(CSV.File("Output/avg_bias_experiments_MNL_full_dynamic_25locs_xizero_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_cr.csv"))
         df_output_zero[!,"xi"] = ["zero" for i in 1:length(Pops_to_consider)]

         df_output_exo = DataFrame(CSV.File("Output/avg_bias_experiments_MNL_full_dynamic_25locs_xiexogenous_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_cr.csv"))
         df_output_exo[!,"xi"] = ["exogenous" for i in 1:length(Pops_to_consider)]

         df_output_endo = DataFrame(CSV.File("Output/avg_bias_experiments_MNL_full_dynamic_25locs_xiendogenous_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_cr.csv"))
         ...
         # Save
         CSV.write("Output/avg_bias_experiments_MNL_full_dynamic_25locs_FEs$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy).csv",  df_all)
         ...
         # Save
         CSV.write("Output/avg_bias_experiments_MNL_full_dynamic_25locs_FEs$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_vertical.csv",  df_horizontal)
         ...
         if layout == "horizontal"
           write("Output/table_FEs$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_div$(Pdims.divide_by_params)_horizontal.tex", Table);
         else
           write("Output/table_FEs$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_div$(Pdims.divide_by_params)_vertical.tex", Table);
        end
      end
   end
end
