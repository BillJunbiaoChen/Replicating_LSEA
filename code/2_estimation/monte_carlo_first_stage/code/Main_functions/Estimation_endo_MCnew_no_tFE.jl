...
for ξ_c in ξ_to_consider
   for (j_id,J) in enumerate(J_to_consider)
      for (pop_id,Pop) in enumerate(Pops_to_consider)
         ...
         Threads.@threads for k in [1,2,3,4,6,7,9,10]
            ...
            Dm_tensor = reshape(Dm,Pdims.T,Pdims.J,n_FEs)
            Dm_tensor_reduced = Dm_tensor[1:Pdims.T-1,1:Pdims.J-1,:]
            global Dm_reduced = reshape(Dm_tensor_reduced,(Pdims.J-1)*(Pdims.T-1),n_FEs)
            ...
            trans_probs_df = DataFrame(CSV.File("Data/simulated_choices_true_prob_mat_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE/prob_mat_J$(J)_Pop$(Pop)_iter$(k).csv"))
            ...
            MNL_df = DataFrame(CSV.File("Data/simulated_choices_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE/simulated_choices_J$(J)_Pop$(Pop)_iter$(k).csv"))
            ...
            freq_probs_df = DataFrame(CSV.File("Data/simulated_freq_probs_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE/simulated_choices_J$(J)_Pop$(Pop)_iter$(k).csv"))
            ...
            # Retrieve data
            dat = DataFrame(CSV.File("Data/simulated_data_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE/simulated_data_J$(J)_Pop$(Pop)_iter$(k).csv"))
            dist_mat = Matrix(DataFrame(CSV.File("Data/simulated_data_full_dynamic_J$(J)_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE/simulated_dist_mat_J$(J)_Pop$(Pop)_iter$(k).csv")))
   ...
   CSV.write("Output/simulated_choices_results_25locs_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE_cr.csv",  df_output_MNL)
   ...
   # Output to CSV
   CSV.write("Output/avg_bias_experiments_MNL_full_dynamic_25locs_xi$(ξ_c)_FE$(Pdims.FEs)_locdummy$(Pdims.include_tau_dummy)_no_tFE_cr.csv",  df_output_quants)
end
