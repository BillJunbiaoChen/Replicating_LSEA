...
##################################################################################################################################
# This file sets up, configures, and executes the airbnb tax counterfactual simulations for a range of counterfactual tax rates.
##################################################################################################################################
...
filepath = '../../../data/simulation_results/gamma_B_' + value + '/counterfactuals/airbnb_tax_proportional/'
if not os.path.exists(filepath):
    os.makedirs(filepath)

print("Result CSVs will be stored at " + filepath)

# Iterations are in increments of 1% of the airbnb tax rate, from 0 to 9%
for tax in range(0, 9, 1):
    ...
    with open('config.json', 'r+') as f:
        ...
        data['counterfactual'] = 'airbnb_city_tax_proportional'
        data['amenity_tax_rate'] = 0
        data['airbnb_tax_rate'] = tax/100

        f.seek(0)
        f.truncate(0)
        json.dump(data, f, indent=4)

    # Making system call to solve for equilibria given config.json which contains the counterfactual airbnb tax rate being tested.
